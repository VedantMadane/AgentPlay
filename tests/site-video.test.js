...
test('PeerTube no-login fixture: progress, cancellation, duplicate reuse and honest private failure', async (t) => {
  ...
  assert.equal(peertubeFixture.license?.id, 7)
  ...
  const cancelDir = fs.mkdtempSync(path.join(os.tmpdir(), 'peertube-cancel-'))
  const cookiesDir = fs.mkdtempSync(path.join(os.tmpdir(), 'peertube-cookies-'))
  t.after(() => {
    fs.rmSync(destDir, { recursive: true, force: true })
    fs.rmSync(cancelDir, { recursive: true, force: true })
    fs.rmSync(cookiesDir, { recursive: true, force: true })
  })
  const produced = path.join(destDir, 'PeerTube-fixture-' + peertubeFixture.shortId + '.mp4')
  const progresses = []
  const ok = new SiteVideoService({
    enginePath: process.execPath,
    spawnImpl: fakeSpawn(({ child }) => {
  ...
  await assert.rejects(
    cancelling.download(peertubeFixture.url, { destDir: cancelDir, signal: controller.signal }),
    /已取消|cancelled/i
  )
